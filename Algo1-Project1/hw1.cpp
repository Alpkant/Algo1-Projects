	...
	*/
	...
	if(strcmp(argv[1], "-full")==0){		//If its fullsort
		if(strcmp(argv[2], "-m")==0)		//If its full sort and merge sort
			boss.mergeSort(cardlist,false,0,cardNumber-1); 

		boss.insertionSort(cardlist,false,cardNumber); //Insertion and full sort
	}else{
		if (strcmp(argv[2], "-m")==0)				//If its filter sort and merge sort
			boss.mergeSort(cardlist,true,0,cardNumber-1); 
		 
		boss.insertionSort(cardlist,true,cardNumber);		//If its filter sort and insertion sort
	}
